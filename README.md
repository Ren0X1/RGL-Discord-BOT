...
- **Stats de Counter-Strike** (`csstats`) — `/cs <url_perfil_steam>`: saca rangos, Leetify rating, puntería, winrate y últimas partidas vía la **API pública de Leetify**, con enlaces a Leetify y csstats.gg. Acepta URLs `/profiles/...`, `/id/...` (con `STEAM_API_KEY`) o el SteamID64.
